...
**✅ VERIFIED**: All steps extracted from actual source code v1.5.8.1
...
### `I set throttle with delay {delay_ms:f} milliseconds`
Configure throttle with fixed delay between requests.

### `I send {count:d} GET requests to "{endpoint}"`
Send multiple GET requests for performance testing.

### `all responses should have status {status:d}`
Validate all responses have same status code.
...
*Judo Framework v1.5.8.1 - All steps verified against source code*